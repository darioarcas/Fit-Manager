...
export const SelectorAlimentos1 = ({setAlimentos1, setearAlimentos, actualizarFormulario, setCambioEnCalculadora, setAlimentos2, setAlimentos3, setAlimentos4, setAlimentos5, setAlimentos6, setAlimentos7}) => {
  ...
  // Dependencias en variables separadas
  const seleccionadosArray = seleccionado.slice(0, 7); // Los primeros 7 elementos
  const cantidadesArray = cantidad.slice(0, 7); // Los primeros 7 elementos
  ...
  const dietaParaReducer = ()=>{
    return {
      ...dieta,
      calculadorasAlimentos: {...dieta.calculadorasAlimentos, selectorAlimentosDesayuno: {
  ...
  useEffect(() => {
    ...
    if(cargaFicha === true && dieta.calculadorasAlimentos){
      ...
      if(dieta.calculadorasAlimentos.selectorAlimentosDesayuno){
        // Ordenar el objeto dieta.calculadorasAlimentos.selectorAlimentosDesayuno de menor a mayor
        Object.entries(dieta.calculadorasAlimentos.selectorAlimentosDesayuno)
        .map(([clave, valor])=>{
